...
class Seller:
    ...
    def get_review_score(self):
        ...
        """
        ndf1=self.data['order_reviews'][['order_id','review_score']].merge(self.data['order_items']).groupby('seller_id',as_index=False).mean()[['seller_id','review_score']]
        ndf2=self.data['order_reviews'][['order_id','review_score']].merge(self.data['order_items']).groupby('seller_id',as_index=False).count()
        ndf3=self.data['order_reviews'][['order_id','review_score']].merge(self.data['order_items'])
        ndf4=ndf3[ndf3['review_score']==5].groupby('seller_id',as_index=False).count()
        ndf5=ndf2.merge(ndf4,how='left',on='seller_id').fillna(0)
        ndf5['share_of_five_stars']=ndf5['order_id_y']/ndf5['order_id_x']
        ndf6=ndf5[['share_of_five_stars','seller_id']]
        nndf2=self.data['order_reviews'][['order_id','review_score']].merge(self.data['order_items']).groupby('seller_id',as_index=False).count()
        nndf3=self.data['order_reviews'][['order_id','review_score']].merge(self.data['order_items'])
        nndf4=nndf3[nndf3['review_score']==1].groupby('seller_id',as_index=False).count()
        nndf5=nndf2.merge(nndf4,how='left',on='seller_id').fillna(0)
        nndf5['share_of_one_stars']=nndf5['order_id_y']/nndf5['order_id_x']
        nndf6=nndf5[['share_of_one_stars','seller_id']]
        return ndf6.merge(nndf6).merge(ndf1)
